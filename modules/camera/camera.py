...
class Camera():
    def __init__(self,
        ...
        self.camera.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0.25) # fixed exposure 0.25, automatic 0.75
        self.camera.set(cv2.CAP_PROP_EXPOSURE, exposure)
        self.camera.set(cv2.CAP_PROP_FPS, fps)
        
        # Set the camera matrix and distortion coefficients
        cam_path = 'modules/camera/config/cam_' + \
                   str(frame_width) + 'x' + str(frame_height) + '.yaml'
        with open(cam_path, "r") as f:
            cameraParameter = yaml.load(f.read(), 
                                        Loader=yaml.Loader)
        ...
        with open('modules/camera/config/detector_params.yaml', "r") as f:
            detectorParameter = yaml.load(f.read(), 
                                          Loader=yaml.Loader)
            arucoParams.adaptiveThreshWinSizeMin = detectorParameter['adaptiveThreshWinSizeMin']
            arucoParams.adaptiveThreshWinSizeMax = detectorParameter['adaptiveThreshWinSizeMax']
            arucoParams.adaptiveThreshWinSizeStep = detectorParameter['adaptiveThreshWinSizeStep']
            arucoParams.adaptiveThreshConstant = detectorParameter['adaptiveThreshConstant']
            arucoParams.minMarkerPerimeterRate = detectorParameter['minMarkerPerimeterRate']
            arucoParams.maxMarkerPerimeterRate = detectorParameter['maxMarkerPerimeterRate']
            arucoParams.polygonalApproxAccuracyRate = detectorParameter['polygonalApproxAccuracyRate']
            arucoParams.minCornerDistanceRate = detectorParameter['minCornerDistanceRate']
            arucoParams.minDistanceToBorder = detectorParameter['minDistanceToBorder']
            arucoParams.minMarkerDistanceRate = detectorParameter['minMarkerDistanceRate']
            arucoParams.cornerRefinementMethod = detectorParameter['cornerRefinementMethod']
            arucoParams.cornerRefinementWinSize = detectorParameter['cornerRefinementWinSize']
            arucoParams.cornerRefinementMaxIterations = detectorParameter['cornerRefinementMaxIterations']
            arucoParams.cornerRefinementMinAccuracy = detectorParameter['cornerRefinementMinAccuracy']
            arucoParams.markerBorderBits = detectorParameter['markerBorderBits']
            arucoParams.perspectiveRemovePixelPerCell = detectorParameter['perspectiveRemovePixelPerCell']
            arucoParams.perspectiveRemoveIgnoredMarginPerCell = detectorParameter['perspectiveRemoveIgnoredMarginPerCell']
            arucoParams.maxErroneousBitsInBorderRate = detectorParameter['maxErroneousBitsInBorderRate']
            ...
            arucoParams.minSideLengthCanonicalImg = detectorParameter['minSideLengthCanonicalImg']
            arucoParams.minMarkerLengthRatioOriginalImg = detectorParameter['minMarkerLengthRatioOriginalImg']
    ...
    def pose_acc(self, 
        ...
        for i in range(6):
            new_pose[i] = np.trunc(pose[i]*10**self.pose_accuracy[i])/10**self.pose_accuracy[i]
    ...
    def _read_pose(self, 
        ...
        if ids is None:
            return np.zeros(6), (cv2.resize(img, (640, 360)) 
                                 if img.shape[0] > 640 
                                 else img)
        
        # Estimate the pose
        rvec, tvec, _ = cv2.aruco.estimatePoseSingleMarkers(corners[0], 
                                                            self.mark_size, 
                                                            self.camera_matrix, 
                                                            self.camera_dist)
        ...
        # Draw the markers
        color_image_result = cv2.aruco.drawDetectedMarkers(img, 
                                                           corners, 
                                                           ids)
        color_image_result = cv2.drawFrameAxes(img, 
                                               self.camera_matrix, 
                                               self.camera_dist, 
                                               rvec[0], 
                                               tvec[0], 
        ...
        return pose, (cv2.resize(color_image_result, (640, 360)) 
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Print the pose
        print("Dx: %3.3f, Dy: %3.3f, Dz: %3.3f, Rx: %3.3f, Ry: %3.3f, Rz: %3.3f" % 
              (pose[0], pose[1], pose[2], pose[3], pose[4], pose[5]), end="\r")
