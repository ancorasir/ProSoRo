...
# Mesh filter
def mesh_filter(nodes: np.ndarray, 
                nodes_list: list, 
                filter_frame: int):
...
class Interface:
    def __init__(self,
        ...
        self.pose_subscriber = PoseSubscriber(address=pub_sub_adress_dict[cam_num]['pose'])
        self.image_subscriber = ImageSubscriber(address=pub_sub_adress_dict[cam_num]['image'])
        ...
        for module_type in module_type_list:
            pth_path = 'models/pths/mvae_' + module_type + '_' + str(recon_pred_scale) + '_' + str(z_coeff) + '_' + str(kl_coeff) + '_' + str(z_dim) + '.pth'
            model = MVAE(
            ...
            # surface triangles
            surf_triangles = np.loadtxt(template_path + 'surf_triangles.txt', delimiter=',').astype(int)
                    ...
                    children=[
                                ...
                                html.Div(
                                    children=[
                                        dash_table.DataTable(
                                            ...
                                            columns=[
                                                {
                                                    'name': ['ArUco Displacement', 'Dx (mm)'], 
                                                    'id': 'Dx'
                                                    },
                                                {
                                                    'name': ['ArUco Displacement', 'Dy (mm)'], 
                                                    'id': 'Dy'
                                                    },
                                                {
                                                    'name': ['ArUco Displacement', 'Dz (mm)'], 
                                                    'id': 'Dz'
                                                    },
                                                {
                                                    'name': ['ArUco Rotation', 'Rx (rad)'], 
                                                    'id': 'Rx'
                                                    },
                                                {
                                                    'name': ['ArUco Rotation', 'Ry (rad)'], 
                                                    'id': 'Ry'
                                                    },
                                                {
                                                    'name': ['ArUco Rotation', 'Rz (rad)'], 
                                                    'id': 'Rz'
                                                    },
                                                ],
                                ...
                                html.Div(
                                    children=[
                                        dash_table.DataTable(
                                            ...
                                            columns=[
                                                {
                                                    'name': ['Predicted Force', 'Fx (N)'], 
                                                    'id': 'Fx',
                                                    },
                                                {
                                                    'name': ['Predicted Force', 'Fy (N)'], 
                                                    'id': 'Fy',
                                                    },
                                                {
                                                    'name': ['Predicted Force', 'Fz (N)'], 
                                                    'id': 'Fz',
                                                    },
                                                {
                                                    'name': ['Predicted Torque', 'Tx (Nmm)'], 
                                                    'id': 'Tx',
                                                    },
                                                {
                                                    'name': ['Predicted Torque', 'Ty (Nmm)'], 
                                                    'id': 'Ty',
                                                    },
                                                {
                                                    'name': ['Predicted Torque', 'Tz (Nmm)'], 
                                                    'id': 'Tz',
                                                    },
                                                ],
                                            merge_duplicate_headers=True,
                        ...
                        html.Div(
                            children=[
                                ...
                                dcc.Graph(
                                    id="mesh", 
                                    figure=go.Figure(data=[mesh_fig],),
                                    style={
                                        'width': '100%',
                                        'height': '100%'
                                        },
        ...
        def update(n_intervals, 
            ...
            # Filter
            if self.filter_on:
                node_pred, nodes_pred_list = mesh_filter(node_pred, self.nodes_pred_list, self.filter_frame)
...
if __name__ == "__main__":
    # Initialize the interface
    interface = Interface(cam_num='cam_0',
                          filter_on=True,
                          filter_frame=5)
    # Run the loop
    interface.run()