...
class MVAE(LightningModule):
    ...
    def __init__(self,
        ...
        for i in range(len(self.x_dim_list)):
            ...
            setattr(self, 
                    f"encoder_{i}", 
                    nn.Sequential(nn.Linear(self.x_dim_list[i], 
                                            self.h1_dim_list[i]),
                                  nn.ReLU(),
                                  nn.Linear(self.h1_dim_list[i], 
                                            self.h2_dim_list[i])))
            # Decoder
            setattr(self, 
                    f"decoder_{i}", 
                    nn.Sequential(nn.Linear(self.z_dim, 
                                            self.h2_dim_list[i]),
                                  nn.ReLU(),
                                  nn.Linear(self.h2_dim_list[i], 
                                            self.h1_dim_list[i]),
                                  nn.ReLU(),
                                  nn.Linear(self.h1_dim_list[i], 
                                            self.x_dim_list[i])))
    ...
    def sample(self, 
        ...
        p = torch.distributions.Normal(torch.zeros_like(mu), 
                                       torch.ones_like(std))
        q = torch.distributions.Normal(mu, 
                                       std)
    ...
    def step(self, 
        ...
        for i in range(len(self.x_dim_list)):
            # MSE loss
            recon_loss_i = F.mse_loss(x_hat_list[i], 
                                      x_list[i], 
                                      reduction="mean")
            # Store the data
        ...
        for i in range(len(self.x_dim_list) - 1):
            ...
            # MSE loss
            pred_loss_0_i = F.mse_loss(x_pred, 
                                       x_list[i + 1], 
                                       reduction="mean")
            # Store the data
        ...
        for i in range(len(self.x_dim_list)):
            # KL divergence
            kl_i = (q_list[i].log_prob(z_list[i]) - p_list[i].log_prob(z_list[i])).mean()
            # Store the data
            logs[f"kl_{i}"] = kl_i
            kl_list.append(kl_i)
        # Average the loss
        ...
        for i in range(len(self.x_dim_list) - 1):
            for j in range(i + 1, len(self.x_dim_list)):
                # MSE loss
                z_loss_list.append(F.mse_loss(z_list[i], 
                                              z_list[j], 
                                              reduction="mean"))
        # Average the loss
        ...
        # Calculate the loss
        loss = self.recon_pred_scale/(1 + self.recon_pred_scale)*recon_loss + \
